...
function decodeBC1Block(inputBuffer: Uint8Array, size: FormatSize): Buffer {
    ...
    var outputBuffer = Buffer.alloc(size.blockWidth * size.blockHeight * rgbaPixelSize)
    for (var i = 0; i < 16; i++) {
...
function getBC3Alpha(code: number, alpha0: number, alpha1: number): number {
    if (alpha0 > alpha1) {
        switch (code) {
...
function decodeBC3Block(inputBuffer: Uint8Array, size: FormatSize): Buffer {
    ...
    for (var i = 0; i < 16; i++) {
		...
		const alpha = getBC3Alpha(code, alpha0, alpha1)

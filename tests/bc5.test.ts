...
test('check BC5 DDS image', () => {
    const data = fs.readFileSync('./tests/resources/dds/bc5.dds')
    const imageInfo = parseDDSHeader(data)

    expect(imageInfo).not.toBe(undefined)
    if (imageInfo) {
        expect(imageInfo.format).toBe('BC5')
...
test('check BC5 KTX image', () => {
    const data = fs.readFileSync('./tests/resources/ktx/bc5.ktx')

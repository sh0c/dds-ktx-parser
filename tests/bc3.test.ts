...
test('check DXT5 DDS image', () => {
    const data = fs.readFileSync('./tests/resources/dds/dxt5.dds')
    const imageInfo = parseDDSHeader(data)

    expect(imageInfo).not.toBe(undefined)
    if (imageInfo) {
        expect(imageInfo.format).toBe('BC3')
...
test('check BC3 DDS image', () => {
    const data = fs.readFileSync('./tests/resources/dds/bc3.dds')
    const imageInfo = parseDDSHeader(data)
...
test('check DXT5 KTX image', () => {
    const data = fs.readFileSync('./tests/resources/ktx/dxt5.ktx')
...
test('check BC3 KTX image', () => {
    const data = fs.readFileSync('./tests/resources/ktx/bc3.ktx')
    const imageInfo = parseKTXHeader(data)
